...
from orgdata.normalize.severity import SEVERITIES, empty_severity_counts, highest_severity, normalize_severity, severity_rank
...
def compact_artifact(artifact):
    ...
    return {
        **id_fields(artifact_id, "artifact"),
...
def compact_package(package):
    ...
    return {
        **id_fields(package_id, "package"),
        ...
        "labels": sorted_unique([package.get("package_type"), package.get("package_group")]),
    }
    ...
    return {
        **id_fields(vulnerability_id, "vulnerability"),
        ...
        "search_text": " ".join([
            ...
            " ".join(vulnerability.get("cwes", [])),
        ]).lower(),
        "labels": sorted_unique([vulnerability.get("severity"), *(vulnerability.get("cwes", []) or [])]),
    }
...
def finding_stats(findings):
    ...
    return {
        ...
        "not_fixable_count": max(len(findings) - fixable_count, 0),
        "security_status": security_status(fixable_count, len(findings)),
        "remediation_available": fixable_count > 0,
...
def build_by_artifact(current_dir, artifacts, artifact_projects, manifests, summary):
    ...
    for artifact_id, artifact in artifacts.items():
        ...
        vulnerable_package_ids = stats["package_ids"]

        record = {
            **id_fields(artifact_id, "artifact-index"),
            "artifact": compact_artifact(artifact),
            "project_ids": project_ids_for(artifact_projects, artifact_id),
            "package_count": len(package_ids),
            "vulnerable_package_count": len(vulnerable_package_ids),
            "package_ids": package_ids,
            "vulnerable_package_ids": vulnerable_package_ids,
            **{k: v for k, v in stats.items() if k not in ("package_ids", "artifact_ids")},
            "search_text": " ".join([artifact_id, artifact.get("artifact_type", ""), artifact.get("artifact_role", "")]).lower(),
            "labels": sorted_unique([artifact.get("artifact_type"), artifact.get("artifact_role")]),
        }

        write_artifact_index(current_dir, artifact_id, record)
        manifests["artifacts"][artifact_id] = {
            **id_fields(artifact_id, "artifact"),
            "partition": f"indexes/by-artifact/{artifact_id}.json",
            "artifact_type": artifact.get("artifact_type", ""),
            "artifact_role": artifact.get("artifact_role", ""),
...
def build_by_package(current_dir, artifact_projects, manifests, summary):
    ...
    for package_file in package_partition_files(current_dir):
        ...
        index = {
            package_id: {
                **package,
                ...
                "fixable_count": 0,
                "not_fixable_count": 0,
                "security_status": "No known vulnerabilities",
        ...
        for package_id, item in index.items():
            ...
            manifests["packages"][package_id] = {
                **id_fields(package_id, "package"),
                "partition": f"indexes/by-package/by-type/{package_file.stem}.json",
                "entity_partition": f"entities/packages/by-type/{package_file.name}",
...
def build_by_vulnerability(current_dir, artifact_projects, manifests, summary):
    ...
    for vuln_file in vulnerability_partition_files(current_dir):
        ...
        index = {
            vulnerability_id: {
                **vulnerability,
                ...
                "fixable_count": 0,
                "not_fixable_count": 0,
                "security_status": "Not fixable",
                "remediation_available": False,
                "fixed_versions": [],
                "severity_counts": empty_severity_counts(),
                "highest_severity": vulnerability.get("severity", "Unknown"),
                "critical_high_count": 0,
            }
        ...
        for vulnerability_id, item in index.items():
            ...
            item["critical_high_count"] = item["severity_counts"].get("Critical", 0) + item["severity_counts"].get("High", 0)
            final_data.append(item)
            manifests["vulnerabilities"][vulnerability_id] = {
                **id_fields(vulnerability_id, "vulnerability"),
                "partition": f"indexes/by-vulnerability/by-year/{vuln_file.stem}.json",
                "entity_partition": f"entities/vulnerabilities/by-year/{vuln_file.name}",
...
def build_remediation(current_dir, artifact_projects, manifests, summary):
    ...
    for findings_file in finding_files(current_dir):
        ...
        for finding in findings:
            ...
            if key not in remediation:
                remediation[key] = {
                    **id_fields(key, "remediation"),
                    "remediation_id": key,
                    "vulnerability_id": finding.get("vulnerability_id", ""),
                    "package_id": finding.get("package_id", ""),
                    "vulnerability_route_id": id_fields(finding.get("vulnerability_id", ""), "vulnerability")["route_id"],
                    "package_route_id": id_fields(finding.get("package_id", ""), "package")["route_id"],
                    ...
                    "severity_counts": empty_severity_counts(),
                    "search_text": " ".join([finding.get("vulnerability_id", ""), finding.get("package_id", ""), " ".join(fixed_versions)]).lower(),
                    "labels": ["fixable"],
                }
    ...
    for item in remediation.values():
        ...
        result.append(item)
        manifests["remediation"][item["remediation_id"]] = {
            **id_fields(item["remediation_id"], "remediation"),
            "partition": "indexes/remediation.json",
            "vulnerability_id": item["vulnerability_id"],
            "package_id": item["package_id"],
            "highest_severity": item["highest_severity"],
            "affected_artifact_count": item["affected_artifact_count"],
        }
...
def build_validation(current_dir, manifests, artifacts):
    ...
    vulnerability_ids = set(manifests["vulnerabilities"].keys())
...
def build_all_indexes(current_dir: Path):
    ...
    summary = {
        ...
        "source_finished_at": (source_run or {}).get("finished_at"),
        "artifact_count": 0,
        "package_count": 0,
    ...
    counts = {
        "by_artifact_count": build_by_artifact(current_dir, artifacts, artifact_projects, manifests, summary),
        "by_package_partition_count": build_by_package(current_dir, artifact_projects, manifests, summary),
        "by_vulnerability_partition_count": build_by_vulnerability(current_dir, artifact_projects, manifests, summary),
        "remediation_item_count": build_remediation(current_dir, artifact_projects, manifests, summary),
    }

    partitions = {
        "package_index_partitions": sorted({item.get("package_type", "unknown") for item in manifests["packages"].values()}),
        "vulnerability_index_partitions": sorted({item.get("partition", "").split("/")[-1].replace(".json", "") for item in manifests["vulnerabilities"].values()}),
        "artifact_index_count": len(manifests["artifacts"]),
        "remediation_index_count": len(manifests["remediation"]),
    }
    ...
    metadata = {
        ...
        "source_finished_at": summary.get("source_finished_at"),
        "generator": "org-data-generator",
        "counts": counts,
        "summary_path": "indexes/index-summary.json",
        "validation_path": "indexes/index-validation.json",
        "manifests": {
            "artifacts": "indexes/manifests/artifacts.json",
            "packages": "indexes/manifests/packages.json",
            "vulnerabilities": "indexes/manifests/vulnerabilities.json",
            "remediation": "indexes/manifests/remediation.json",
            "partitions": "indexes/manifests/partitions.json",
        },
    }

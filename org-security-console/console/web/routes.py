...
def paginated(items):
    ...
    page_data["prev_args"] = dict(base_args, page=page_data["prev_page"], per_page=page_data["per_page"])
    page_data["next_args"] = dict(base_args, page=page_data["next_page"], per_page=page_data["per_page"])
    return page_data
...
def artifacts():
    ...
    page_data = paginated(queries().artifacts(search or None))
    return render_template("artifacts.html", artifacts=page_data, page=page_data, search=search, title="Artifacts")


@bp.route("/artifact/<route_id>")
def artifact_detail(route_id):
    return render_template("artifact_detail.html", detail=queries().artifact_detail(route_id), title="Artifact Detail")
...
def vulnerability_detail(route_id):
    return render_template("vulnerability_detail.html", detail=queries().vulnerability_detail(route_id), title="Vulnerability Detail")
...
def packages():
    ...
    page_data = paginated(queries().packages(package_type or None, search or None, status or None))
    return render_template(
        "packages.html",
        packages=page_data,
        page=page_data,
...
def package_detail(route_id):
    return render_template("package_detail.html", detail=queries().package_detail(route_id), title="Package Detail")


@bp.route("/data-health")
def data_health():
    return render_template("data_health.html", health=queries().run_health(), title="Data Health")

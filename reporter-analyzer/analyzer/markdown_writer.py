def write_markdown_report(summary, path):
    ...
    lines.append("**Artifact Security Analysis Report**")
    lines.append("")
    ...
    if artifact.get("artifact_type") == "container_image":
    ...
    if artifact.get("artifact_type") == "source_project":
        lines.append("**Source Details**")
        lines.append("")
        lines.append(f"- Source path: `{source.get('source_path', '')}`")
        lines.append(f"- Repository: `{source.get('source_repository', '')}`")
        lines.append(f"- Branch: `{source.get('source_branch', '')}`")
        lines.append(f"- Tag: `{source.get('source_tag', '')}`")
        lines.append(f"- Commit: `{source.get('source_commit', '')}`")
        lines.append("")

    lines.append("**Scan Timing**")
    lines.append("")
